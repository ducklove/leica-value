#!/usr/bin/env python3
"""Build the static GitHub Pages artifact for the Leica value tracker."""
...
def build_brand_hero(*, title: str, subtitle: str, updated: str | None = None) -> str:
    updated_html = f'<p class="site-updated">최종 업데이트: {escape(updated)}</p>' if updated else ''
    return f"""
    <div class=\"hero-banner hero-banner--brand\">
      <div class=\"hero-banner__halo\" aria-hidden=\"true\"></div>
      <div class=\"container hero-banner__content\">
        <div class=\"hero-brand-mark\" aria-hidden=\"true\">L</div>
        <div class=\"hero-banner__copy\">
          <h1 class=\"site-title\">{escape(title)}</h1>
          <p class=\"site-subtitle\">{escape(subtitle)}</p>
          {updated_html}
        </div>
      </div>
    </div>"""
...
def build_home_page(catalog: dict[str, Any], base_url: str) -> str:
    ...
    cards = []
    rare_watch_cards = []
    image_url = f'{base_url}/assets/leica-hero.svg' if base_url else 'assets/leica-hero.svg'
    ...
    for category in catalog['categories']:
        ...
        for product in sort_products(category['products'], category['id']):
            ...
            if samples and samples[0].get('image'):
                ...
                thumb = '<div class="product-card__thumb-placeholder" aria-hidden="true">Leica</div>'
...
<html lang=\"ko\">
{head_block(title='라이카 중고 시세 트래커', description=description, canonical=canonical, image_url=image_url, extra_meta=extra_meta)}
<body data-page=\"catalog\">
  <header class=\"site-header\">
{build_brand_hero(title='라이카 중고 시세 트래커', subtitle='eBay 현재 매물 기준 시세 (배송비 포함 USD)', updated=updated)}
  </header>
  ...
  <main class=\"container\">
    <section class=\"catalog-toolbar\" aria-label=\"시세 탐색\">
      <div class=\"catalog-toolbar__copy\">
        <span class=\"section-kicker\">Leica used value tracker</span>
...
def product_image(product: dict[str, Any], base_url: str) -> str:
    ...
    if base_url:
        return f'{base_url}/assets/leica-hero.svg'
    return '../assets/leica-hero.svg'
...
def build_product_reference_cards(product: dict[str, Any], category: dict[str, Any], asset_prefix: str = '../') -> str:
    cards = [
        ...
        (
            ...
            'Leica Camera AG',
            f'{product["name_ko"]}가 속한 라이카 시스템의 공식 제품 정보와 브랜드 아카이브를 확인할 수 있습니다.',
            True,
        ),
        (
            'https://wiki.l-camera-forum.com/',
            'Leica Wiki',
            '바디/렌즈 세대와 변형, 사양 차이를 빠르게 대조하기 좋습니다.',
            True,
        ),
    ]
    ...
    if is_lens_category(category['id']):
        cards.insert(
            ...
            (
                ...
                'CameraQuest Leica Guide',
                'M 렌즈와 바디 세대별 특징, 사양 차이를 빠르게 훑어보기 좋습니다.',
                True,
            ),
        )
        cards.insert(
            3,
            (
                'https://www.red-dot-forum.com/',
                'Red Dot Forum',
                'Leica 렌즈 운용 경험과 실제 사용자 피드백을 확인하기 좋습니다.',
                True,
            ),
        )
...
def build_product_page(
    ...
    return f"""<!DOCTYPE html>
<html lang=\"ko\">
{head_block_product(title=f"{product['name_ko']} - 라이카 중고 시세", description=description, canonical=canonical, image_url=image_url, extra_meta=extra_meta)}
...
def build_resources_page(base_url: str) -> str:
    ...
    image_url = f'{base_url}/assets/leica-hero.svg' if base_url else 'assets/leica-hero.svg'
    description = 'Leica 바디와 렌즈의 계보, 리뷰, 역사 자료를 볼 수 있는 참고 사이트 모음입니다.'
    schema = {
        '@context': 'https://schema.org',
        '@type': 'CollectionPage',
        'name': '참고 사이트 링크',
    ...
    resources = [
        (
            'Leica Camera AG',
            'https://leica-camera.com/en-int',
            '공식 제품 정보, 신제품 발표, 서비스 및 브랜드 아카이브를 확인할 수 있습니다.',
        ),
        (
            'Leica Wiki',
            'https://wiki.l-camera-forum.com/',
            'Leica 바디와 렌즈의 세대별 차이, 스펙, 변형 정보를 대조하기 좋습니다.',
        ),
        (
            'Red Dot Forum',
            'https://www.red-dot-forum.com/',
            'Leica 사용자 리뷰, 렌즈 선택 팁, 실전 운용기를 확인하기 좋습니다.',
        ),
        (
            'CameraQuest Leica Guide',
            'https://www.cameraquest.com/leica.htm',
            'M 시스템과 고전 Leica 렌즈 계보를 빠르게 읽어보기 좋습니다.',
        ),
    ]
    cards = []
    ...
    return f"""<!DOCTYPE html>
<html lang=\"ko\">
{head_block(title='참고 사이트 링크 - 라이카 중고 시세 트래커', description=description, canonical=canonical, image_url=image_url, extra_meta=extra_meta)}
<body data-page=\"resources\">
  <header class=\"site-header\">
{build_brand_hero(title='참고 사이트 링크', subtitle='시세 숫자 외에 계보, 사양, 역사 자료를 같이 볼 때 유용한 Leica 레퍼런스입니다.')}
  </header>
  ...
  <main class=\"container page-main\">
    <section class=\"info-card info-card--wide\">
      ...
      <h2 class=\"section-heading\">라이카 자료실</h2>
...
def copy_assets(output_dir: Path) -> None:
    ...
    shutil.copy2(BRAND_OG_ASSET, output_dir / 'assets' / 'leica-hero.svg')
